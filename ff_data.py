...
class ff_data(object):
    ...
    __csv_column = ["Key", "On", "Last modified (days)", "Last evaluated (days)", "Created", "Updated", "Owner", "Description", "Tags"]
    ...
    def __create_csv(self, rows, environment):
        today = datetime.now()
        path = ".//data//"
        self.__create_folder(path)

        filename = "{0}//{1}-feature-flags-{2:%Y-%m-%d}.csv".format(path, environment, today)
    ...
    def __get_last_evaluated(self, flag_key):
        url = self.__params_flag_status.format(self.__config.project_key, self.__config.environment_key, flag_key)
    ...
    def __store_feature_flag_data(self, flags, rows):
        for flag in flags:
            key = flag["key"]
            description = flag["description"]
            tags = flag["tags"]
            owner = self.__get_team_name(flag["_maintainer"]["email"])
            active = flag["environments"][self.__config.environment_key]["on"]
            created_date = self.__get_date_from_milliseconds(flag["creationDate"])
            updated_date = self.__get_date_from_milliseconds(flag["environments"][self.__config.environment_key]["lastModified"])

            last_requested = self.__get_last_evaluated(key)
            ...
            rows.append([key, active, days_since_update, days_last_evaluated, created_date.strftime('%d/%m/%y'), updated_date.strftime('%d/%m/%y'), owner, description, tags])
    ...
    def extract_audit_data(self):
        environment = self.__config.environment_key
        data = self.__get_feature_flags(environment)

        csv_rows = []
        self.__store_feature_flag_data(data, csv_rows)
        return self.__create_csv(csv_rows, environment)